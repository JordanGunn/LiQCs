...
class LidarSummary:
    ...
    def guid(self):
        guid = re.search(R'(project ID GUID data 1-4:)\s+(\d{8}-\d{4}-\d{4}-\d{4}-\d{12})', self.info)
        if guid:
            self.info_dict['GUID'] = guid[2]
            return guid[2]
            

    def system_id(self):
        sys_id = re.search(R'(system identifier:)\s+([^\n]+)', self.info)
    ...
    def last_density(self):
        dens = re.search(R'(point density: all returns \d+.\d\d last only)\s+(\d+.\d\d)', self.info)
